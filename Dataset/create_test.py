...
def merge_greene():
    ...
    interactions = pd.read_parquet(get_greene_path('interactions_test'))
    tracks = pd.read_parquet(get_greene_path('tracks_test'))
    ...
    music_data.to_parquet(get_greene_user_path('test'))
    print('Write Complete!')

def merge_hdfs(spark):

    interactions = spark.read.parquet(get_hdfs_path('interactions_test'))
    tracks = spark.read.parquet(get_hdfs_path('tracks_test'))
    ...
    music_data.write.parquet(get_hdfs_user_path('test'))
    print('Write Complete!')

if __name__ == '__main__':

    if NETID.endswith('_nyu_edu'):
        spark = SparkSession.builder.appName('Create Test Data').getOrCreate()
        merge_hdfs(spark)
    else:
        merge_greene()