...
def main(spark, userID, filename = 'interactions_train_small'):
    ...
    if filename[-6:] == "_small":
        train_file_path = f'hdfs:/user/{userID}/interactions_train_ordered_small_filtered.parquet'
        val_file_path = f'hdfs:/user/{userID}/interactions_val_ordered_small_filtered.parquet'
        track_file_path = 'tracks_train_small'
    else:
        train_file_path = f'hdfs:/user/{userID}/interactions_train_ordered_filtered.parquet'
        val_file_path = f'hdfs:/user/{userID}/interactions_val_ordered_filtered.parquet'
    ...
    train = spark.sql('SELECT i.user_id, i.recording_mbid, i.timestamp FROM interactions_with_rowid as i INNER JOIN split_index as s ON i.user_id = s.user_id WHERE i.interaction_number < s.split_index')
    print("Total Rows in Train Data:", train.count())

    val = spark.sql('SELECT i.user_id, i.recording_mbid, i.timestamp FROM interactions_with_rowid as i INNER JOIN split_index as s ON i.user_id = s.user_id WHERE i.interaction_number >= s.split_index')
    print("Total Rows in Val Data:", val.count())

    train.write.parquet(train_file_path)
    val.write.parquet(val_file_path)
